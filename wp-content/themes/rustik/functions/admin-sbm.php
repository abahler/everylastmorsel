			...
			*/
					...
					if ( ( $type == '' || in_array( $type, $tax_keys ) ) ) {
						...
						if ( $type_tax != '' ) {
							...
							if ( ( isset( $taxonomy_archive_query_obj->name ) && isset( $taxonomy_archive_query_obj->slug ) &&isset( $taxonomy_archive_query_obj->term_id ) ) && ( is_tax( $taxonomy_archive_query_obj->name, $taxonomy_archive_query_obj->slug ) ) && ( $id == $taxonomy_archive_query_obj->term_id ) ) { $sentinel = true; } // End IF Statement
							else {
								$sentinel = false;
							}
							if ( ! $sentinel ) {
