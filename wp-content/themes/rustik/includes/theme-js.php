	...
	function woothemes_add_javascript( ) {
		...
		if ( (is_home() || is_front_page()) && !is_paged() ) {
			...
			if( isset( $woo_options[ 'woo_slider_autoheight' ] ) ) 
				$woo_slider_autoheight = $woo_options[ 'woo_slider_autoheight' ];
			else 
				$woo_slider_autoheight = '';
			
			if( isset( $woo_options[ 'woo_slider_effect' ] ) ) 
				$woo_slider_effect = $woo_options[ 'woo_slider_effect' ];
			else 
				$woo_slider_effect = '';
				
			if( isset( $woo_options[ 'woo_slider_random' ] ) ) 
				$woo_slider_random = $woo_options[ 'woo_slider_random' ];
			else 
				$woo_slider_random = '';
				
			if( isset( $woo_options[ 'woo_slider_hover' ] ) ) 
				$woo_slider_hover = $woo_options[ 'woo_slider_hover' ];
			else 
				$woo_slider_hover = '';
				
			if( isset( $woo_options[ 'woo_slider_speed' ] ) ) 
				$woo_slider_speed = $woo_options[ 'woo_slider_speed' ];
			else 
				$woo_slider_speed = 1;
			
			if( isset( $woo_options[ 'woo_slider_pagination' ] ) ) 
				$woo_slider_pagination = $woo_options[ 'woo_slider_pagination' ];
			else 
				$woo_slider_pagination = '';
				
			$params = array(
				'enabled'		=> $enabled,
				'loading_image' => get_template_directory_uri() . '/images/loading.png',
				'autoheight' 	=> $woo_slider_autoheight,
				'effect' 		=> $woo_slider_effect,
				'randomize' 	=> $woo_slider_random,
				'hoverPause'	=> $woo_slider_hover,
				'slideSpeed'	=> $woo_slider_speed * 1000,
				'generatePagination' => $woo_slider_pagination
			);
			
			if ( isset( $woo_options[ 'woo_slider_auto' ] ) && $woo_options[ 'woo_slider_auto' ] != '' && $woo_options[ 'woo_slider_auto' ] == "true") :
